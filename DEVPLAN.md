...
- [x] **The role answered on 2026-09-05, on CH-0002 and CH-0003: six stumbles and five.** They are the shape the entry asked for — quoted, first-pass, no rewrite proposed, and none of them reachable by a reader who can see the English. Three are pronouns with no findable referent (*«Binta la sentì salire lungo la torre»* — the rhyme, the laugh, or something else); one is the English word `misread` left standing inside Italian prose, which a person reading CH-0001 had caught independently; one is *«liscio come un gradino»*, read as a comparison of smoothness to a step before the sense arrives. On CH-0001 it spent its reasoning ceiling and read nothing, which is the third distinct way this role has failed and the second time on the ceiling.
- [x] Measured on landfall's CH-0001 against the constructions the human readers found. **It answered on 2026-09-05, once the reasoning ceiling was fixed**, and the first thing it reported was that class exactly: *«Date loro un naufragio da mettere sotto ferita»*, a word-for-word rendering of *to put under 'wound'*, `blocking`. Repaired to *«da scrivere sotto la voce 'ferita'»*. Three more on the same chapter, four on CH-0002, five on CH-0003 — an English word left standing inside Italian prose, three pronouns with no findable referent, and a simile read the wrong way round. Two attempts on 2026-09-04, two different failures: the first found no agent at all (see *A role the engine has is a role the project can call*), the second returned `0 output token(s) after spending 32000 on reasoning` — `ReasoningCeilingSpent`, on a role declared at 3000 output tokens, which is the ceiling defect recorded in *What the engine calls an output budget never reaches the provider*. So the pass that exists to read the Italian has read none of it, twice, for two reasons that have nothing to do with each other.
...
**This is CH-0001 after the rebuild.** Both languages were redone on 2026-09-04 and 05, the English against the three new craft rules, the Italian against a corrected glossary. The rebuild removed the effect-sentences and replaced «il Muro»; the interference layer survived it untouched, which is why the earlier measurement — nine effect-sentences left in the English — was counting the wrong thing.

**The monolingual reader already found one of them, and the engine threw it away.** `books/BOOK-0001/translations/it/reviews/CH-0001.json`, finding `R-04`, kind `readability`, origin `reader`, severity **`note`**: «senza di esso l'aria della palude le sedeva sul petto come un cappotto bagnato a metà guardia». The repair trigger keeps two severities:

```python
actionable = [row for row in review["findings"] if str(row.get("severity")) in {"blocking", "warning"}]
```

A `note` is written to the review file and reaches no repair. The role built to find exactly this defect found it, filed it correctly, and the filing is what discarded it.

**The severity scale asks a question a calque cannot fail.** `locale-reader.md` defines the grades by what happened to the reader: `blocking` is a sentence it could not understand, `warning` one it read twice, `note` "a word or phrase that is wrong but did not stop you". A calque is grammatical and does not stop a fluent reader — that is its definition and the reason the role exists. So the scale sorts the defect class it was built for into the one grade that is never repaired. Of the four stumbles the reader returned on CH-0001, the two graded `warning` were repaired and the `note` was not.

**The bound is an order of magnitude under the density.** `LOCALE_READER_MAX_FINDINGS = 6` and `CRITIC_MAX_FINDINGS = 4`, so at most ten findings per pass. The two paragraphs above carry ten on their own, and the chapter is forty-five paragraphs. The bound itself is not the error — an unbounded question was measured to return nothing at all in four attempts out of four — but a per-chapter bound of six cannot describe a defect that occurs several times per paragraph.

**The repair is bilingual, and is told something false about where the finding came from.** `_repair_translation` builds a capsule carrying `source_markdown`, and its reason reads `"a critic read this translation against its source"` for every finding in the list, including the ones whose `origin` is `reader` — a role defined by never having seen the source. So the sentence that is defective *as Italian* is rewritten by a role with the English in front of it, which is the condition under which the calque parses. `_locale_reader_capsule` already states this in its own docstring as the reason the reader is denied the source; the repair does not honour it.

**The prose under discussion was translated with no reasoning effort at all.** `ROLE_SPECS["translator"] = ("all", "low", 7)` is the default a new project gets, and landfall overrides it: `roles.translator` pins `openrouter/z-ai/glm-5.3-flash` at `variant: high`. That pin was inert until 09:42 this morning, when the variant ladder stopped emitting `reasoningEffort` — a key the OpenRouter provider does not read — and started emitting `reasoning: {effort}`. The two sentences quoted above entered the file at 23:29 on 2026-09-04 (`leghe di fango`, TXN-0312) and 03:35 on 2026-09-05 (`l'aria le sedeva sul petto`, TXN-0332), both before the fix. Landfall's `opencode.json` has since been synced and carries the working key, so re-running the same pin today already sends an effort the provider acts on, which the text on disk was never given. That does not excuse the defects — it means the measurement of how bad the model is at this has not actually been taken yet.

**And the model was never chosen for this job.** `glm-5.3-flash` is a flash-tier model translating literary prose, pinned because it is the writer's model. `bakeoff` compares models on a chapter, but it drafts with the writer and there is no equivalent for the translator, so the one role whose output a person could not read is the one no comparison has ever covered. `model-score-ranking.csv` cannot decide it either: it scores agentic and coding indices, which say nothing about Italian prose. The catalogue already configures heavier siblings of the incumbent — `z-ai/glm-5.3` at `max`, `qwen3.8-max`, `deepseek-v4-pro-0813`, `kimi-k3` — at output prices from $0.86 to $15.00 per million tokens against the incumbent's tier, and a chapter is about six thousand output tokens.

**The rule against this is already written, in both places the translator reads.** `assets/prompts/translator.md`: *Translate the meaning, never the words. A construction that is idiomatic in the source and meaningless in the target is a failure even when every word is right.* And landfall's own `translations/it/style.md` has a section headed **Contro il calco**: *Un'espressione idiomatica inglese si rende con l'equivalente italiano, mai parola per parola* — in a file the translator's prompt calls binding rather than advisory, alongside four more rules of the same kind that the quoted paragraphs also break. Ten calques were produced against two statements of the rule. **A third statement is not the lever.** What the pipeline has never had is a pass that checks the rule after the prose exists, in the language the prose is in.

**The translator's prompt carries one line on the subject** — *Translate the meaning, never the words. A construction that is idiomatic in the source and meaningless in the target is a failure even when every word is right* — which states the rule and gives no procedure for applying it and no test for having failed it.

**The glossary can hold two rows that contradict each other, and does.** Landfall's row 105: `the Wall (the tidal bore) / tide-wall → il Cavallone`, with **Never «il Muro»** written into the note and the reason recorded. Row 134: `Wall's clear / the watch formula → «Il Muro è libero»`, with *fissa ad ogni ricorrenza*. Nothing reads the glossary against itself, so the review that is meant to remove «il Muro» from the five chapters that still carry it would put it back through the watch formula.

**The published work says the same thing, and corrects one part of the fix.** Toral's *Post-editese: an Exacerbated Translationese* (arXiv 1907.00900), across five language directions, measures post-edited text as simpler, more normalised and carrying **more source-language interference than translation done from scratch**; the literary follow-up (arXiv 2504.03045) states that the features of post-edited translations sit closer to the machine output than to human translation. Repairing a sentence with the source on the page produces the interference profile this entry exists to remove, which is what CH-0001 shows by hand. But *Iterative Translation Refinement with LLMs* (arXiv 2306.03856) finds that multi-turn refinement lowers string-similarity scores while neural metrics and human readers rate the result equal or better — readers called the refined text more fluent and natural than the initial translation and than the human references — **and its ablation finds that anchoring the refinement to the source matters**: dropping the original entirely costs fidelity. So the monolingual rewrite is right and the meaning check after it is not a nicety, it is the half that holds fidelity, and it is written below as a gate rather than a report. Two consequences follow. The measure of a translation cannot be similarity to a reference, because refinement lowers exactly that while raising judged quality — it has to be the reader's defect count. And the yardstick for naturalness is text originally written in the target language (*A Natural Diet*, Findings of ACL 2022, on machine output mirroring source constructs), which is the reader's second question in the first bullet below.

**Fix — the division of labour the pipeline is missing.** Meaning is guarded bilingually and language is guarded monolingually, and today both jobs are done with the source on the page. No part of this is a list of forbidden words: a word list changes with every book and every chapter, and none of the ten defects above would appear on one written in advance. What generalises is the class of interference and the question that exposes it.

- **The reader grades by defect class, not by reading speed.** It keeps the stumble it has, and gains one question for any sentence it doubts: would a writer of this language, writing this scene with no source in front of them, have produced this sentence? A `no` is `warning` however smoothly it read. `note` keeps its meaning for everything else.
- **The reader covers the chapter.** The bound stays per call, because the measurement that set it stands; the chapter is read in slices so the bound stops being the chapter's limit, and the pass reports what it covered.
- **A monolingual revision runs on every chapter, as a stage of the normal path.** A new role, `locale-reviser`, monolingual by construction the way the reader is: it receives the translated text and the locale style, and never `source_markdown`. It mirrors the prose side, where the reviser is not the writer, and it is the stage this pipeline is missing — translation is followed by a bilingual review and never by a revision in the target language alone. It is not conditional on the reader raising findings: a bound of six over forty-five paragraphs cannot be the trigger for a defect that occurs several times a paragraph. The critic then verifies the revision did not move a fact, which is the one judgement it is structurally good at. This is the change that reaches the initial translation rather than the read-back, and it costs one call per chapter.
- **The translator is given the classes and a procedure.** The classes are properties of language pairs, not of this book: a verb that does not take this noun in the target; a word whose commonest sense in the target is not the sense meant; a source adjective used as a target noun; a resultative or other construction the target does not build; a term of art borrowed from the wrong trade; a measure or unit the target expresses another way. The procedure is the reader's test applied by the writer: after a paragraph is written, read it with the source covered, and rewrite any sentence whose sense you can only recover by remembering the source.
- **The translator is chosen the way the writer is, and by the reader that judges it.** `bakeoff` grows a translator route: one chapter, N models, identical capsule, nothing promoted. The drafts are scored by the monolingual reader on the grading this entry installs, so the comparison is a count of language defects per thousand words rather than a preference. The candidates stay flash-tier by the user's decision of 2026-09-05, which the catalogue supplies four of: `z-ai/glm-5.3-flash` (the incumbent), `deepseek/deepseek-v4-flash-0731`, `google/gemini-3.7-flash` and `qwen/qwen3.8-flash`, all at `default_effort: high`. One consequence to hold: `roles.translation-critic` already pins `gemini-3.7-flash`, and a critic on the translator's model is refused when the config is read, so gemini winning moves the critic. The default effort in `ROLE_SPECS` is set from the same table rather than argued.
- **The glossary is read against itself**, mechanically and with no model call: a row whose target text contains a rendering another row forbids, and two rows whose source terms overlap with incompatible targets.

**Tasks:**
- [x] The reader's second question, and severity by defect class: a sentence no writer of the language would have produced is `warning`, whatever the reading speed. `natural: false` floors `note` to `warning` in code as well, so a reader still grading by reading speed cannot bury it
- [x] Test: a stumble the reader grades on the new question reaches the repair, and a `note` still does not — 5 tests, including that a grade above the floor is not lowered to it
- [x] The reader reads a chapter in slices of 12 paragraphs, each call bounded as now, the first also carrying the whole chapter for the summary. **The reviser is sliced too**, which the first real run showed was the larger half: told to read the whole chapter and not stop at the findings, it rewrote exactly the three sentences it had been handed
- [x] `locale-reviser`: role spec, budget, prompt, generated agent, and a capsule that cannot carry the source. It keeps the glossary, unlike the reader — the reader is denied it so an unreadable term is reported rather than excused as agreed, and a reviser that renames the book's fixed terms trades one defect for a worse one
- [x] It runs on every chapter in `translate next`, `translate run` and `translate review`, after the translation validates and before the bilingual review. The order is now `translator → locale-reader → locale-reviser → translation-critic`
- [x] **Gate:** two halves, failing closed. The mechanical one is the validation the translation itself passed — numbers against the source, structure, length ratio, leakage — plus the glossary check as a regression. The bilingual one is `revision-check`, the critic's pin under its own prompt, given the source and the `before`/`after` pairs and nothing else; it is not asked at all when the mechanical half already refused, or when nothing changed
- [x] Test: the revision capsule contains no `source_markdown`, a digit the source lacks is refused with no model call, and a number spelled out in words is refused by the bilingual half — the second case is why one gate was not enough
...
**What it costs is not disk.** The glossary is read into every translator call, every critic call and every repair, so a row is context the model pays for on every chapter for the rest of the book. It is also the authority the critic cites, so an ordinary word promoted to a fixed rendering turns a defensible synonym elsewhere into a finding. And the mechanical check reads it: twelve rows had to be marked `→?` on 2026-09-04 because they were phrases a translator is meant to read rather than reproduce, which is a symptom of the same thing — sentences entering a term list.

**What belongs in a glossary is a term whose rendering must not vary**: a proper noun, a coined term of the world, a title, a formula repeated verbatim. What belongs in a chapter's record is everything the translator learned while doing that chapter. The engine has both places and writes to one.
...
- [x] A rewritten sentence the reader named that comes back unchanged is recorded as not landed, the way a repair already is — **2026-09-09.** The second rewrite is handed the reader's own sentences as evidence that the first pass failed, and whether those sentences changed was never checked: a writer that rewrote everything around them and left them word for word was counted as having revised the passage. Checked against the text rather than against the next reader, which is stronger than what the critic can do — the sentence was quoted out of this chapter, so after a rewrite it is either still there word for word or it is not. Counted only when the rewrite came back and was kept, the same condition the repair's not-landed is counted under: a pass that returned the chapter unchanged is already reported as a refusal, and counting it twice would say the writer claimed something it never claimed. Recorded in the review file as `rewrites_not_landed`, on stderr, and in the `translate review` summary, sticky across passes. Six tests; suite green at 913 passed, 405 subtests (era 907)
...
- [x] Suite green. Reinstall, commit & push — **the tick was missed on the day.** The work shipped 2026-09-06 and the suite has been green on every run since; recorded here on 2026-09-09 rather than left dangling, and the number is not stated because the number from that day was never written down
...
- [x] **The match is `_term_pattern`, not equality — corrected after the first build shipped and flagged nothing.** The first version compared the reader's name to the glossary term with `casefold()`, and CH-0003's next run marked none of its three names. The reader names a thing as the text names it: it returned `i keelback`, `un Silenzioso` and `la gabbia`, against rows holding `keelback` and `i Silenziosi`. The tests passed because they fed bare terms invented for the test, so they exercised a matcher no real run could reach — the defect was in what the test fed, not in what it asserted. `_settled_term` now uses the same matcher the compliance check uses, which drops the leading article and leaves the ending open, and that is what carries `i Silenziosi` onto `un Silenzioso`
- [x] Test: a kept term and a rendered term both raise the question; a name the glossary never settled stays an ordinary defect and still reaches the repair; an author question reaches no repair at all. **With the shapes the role returns**: `i keelback` and `un Silenzioso` are questions, `la gabbia` is a defect. Checked against landfall's real glossary as well as the fixture — six names, including two that must not be marked
